 ...
 * STEP3: rearrange bTree b
 ...
 END
 ***********************************************************/
memmove(&b->keysAndKids.keys[pos+1], &b->keysAndKids.keys[pos], sizeof(*(b->keysAndKids.keys)) * (b->keysAndKids.numKeys-pos));//mid 값을 저장할 공간 만들기 
memmove(&b->keysAndKids.kids[pos+2], &b->keysAndKids.kids[pos+1], sizeof(*(b->keysAndKids.keys)) * (b->keysAndKids.numKeys-pos));//b2 를 연결할 공간 만들기
b->keysAndKids.keys[pos]=mid;//만들어진 key저장공간에 mid 저장
b->keysAndKids.kids[pos+1]=b2;//만들어진 kids공간에 b2연결
b->keysAndKids.numKeys++;//key 보유수 증가
}
 ...
 *STEP3:                                       *
 b->numKeys++;                                *
 ***********************************************/
 memmove(&b->keysAndKids.keys[pos+1], &b->keysAndKids.keys[pos], sizeof(*(b->keysAndKids.keys)) * (b->keysAndKids.numKeys-pos));// pos위치에 key값을 저장하기 위해  메모리 이동
 b->keysAndKids.keys[pos]=key;//만들어진 공간에 key값저장
 b->keysAndKids.numKeys++;//key보유수 증가
